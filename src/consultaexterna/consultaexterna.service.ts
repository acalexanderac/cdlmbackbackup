...
paciente,
  ...
  const consultaExterna = await this.consultaexternaRepository.find();

  // Using xlsx library to generate XLSX file
  const worksheetXLSX = XLSX.utils.json_to_sheet(consultaExterna, {
   header: [
  'id',
  'dpi',
  'fechaCreacion',
  ...
  'flujo',
  'labiosmenoresnormal',
  'labiosmenoresanormal',
  'labiosmenores',
  'labiosmayoresnormal',
  'labiosmayoresanormal',
  'labiosmayores',
  'aparatourinarionormal',
  'aparatourinarioanormal',
  'aparatourinario',
  'fondodesaconormal',
  'fondodesacoanormal',
  'fondodesaco',
  'cupulavaginalnormal',
  'cupulavaginalanormal',
  'cupulavaginal',
  'cistocele1',
  'cistocele2',
  'cistocele3',
  ...
  XLSX.utils.book_append_sheet(workbookXLSX, worksheetXLSX, 'ConsultaExterna');
  ...
  const consultaExterna = await this.consultaexternaRepository.find({
    where: {
      fechaConsultaexterna: Between(firstDayOfMonth, lastDayOfMonth),
    },
  });
...
async generateReportSemanal(): Promise<Buffer> {
  ...
  const consultaExterna = await this.consultaexternaRepository.find({
    where: {
      fechaConsultaexterna: Between(firstDayOfWeek, lastDayOfWeek),
    },
  });
